...
* Introduction
  Pie is a very simple Package Installer for Emacs. It has much less features than elpaca, straight etc.
...
** Minimal version of Emacs that pie supports
   I developed, tested and used pie with Emacs 28.2, so I wrote down that in the =Package-Requires:= , but it should work together with older versions (maybe with some small changes).
* License
  GPLv3
